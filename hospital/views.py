...
def discharge_patient_view(request,pk):
    ...
    if request.method == 'POST':
        ...
        #making patient discharge record
...
@user_passes_test(is_doctor)
def doctor_dashboard_view(request):
    current_doctor = models.Doctor.objects.get(user_id=request.user.id)
    department = current_doctor.department
    doctors_in_dept = models.Doctor.objects.filter(department=department, status=True).values_list('user_id', flat=True)
    
    # Cards
    patientcount = models.Patient.objects.filter(status=True, assignedDoctorId__in=doctors_in_dept).count()
    ...
    # Appointments and their patients
...
def doctor_view_patient_view(request):
    ...
    doctors_in_dept = models.Doctor.objects.filter(department=department, status=True).values_list('user_id', flat=True)
    patients = models.Patient.objects.filter(status=True, assignedDoctorId__in=doctors_in_dept)
    return render(request, 'hospital/doctor_view_patient.html', {'patients': patients, 'doctor': doctor})
...
def search_view(request):
    ...
    patients = models.Patient.objects.filter(
        status=True,
        assignedDoctorId__in=doctors_in_dept
    ).filter(
